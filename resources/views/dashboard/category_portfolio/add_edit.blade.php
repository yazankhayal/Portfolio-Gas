...
@section('title')
    {{$lang->category_portfolio}}
@endsection

@section('css')
@endsection

@section('create_btn'){{route('dashboard_category_portfolio.index')}}@endsection
                        ...
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active" id="home" role="tabpanel"
                                ...
                                <form class="ajaxForm users" enctype="multipart/form-data" data-name="users"
                                      action="{{route('dashboard_category_portfolio.post_data')}}" method="post">
                                    {{csrf_field()}}
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <a href="{{route('dashboard_category_portfolio.index')}}" class="btn btn-default">
                                            {{$lang->Close}}
                                        </a>
                            ...
                            @if($langauges->where('dir','!=',$select_lan_choice->dir)->count() > 0)
                                ...
                                <hr>
                                <form class="ajaxForm translate" data-name="translate" action="{{route('dashboard_category_portfolio_translate.post_data')}}" method="post">
                                    <div class="modal-body row">
                                        {{csrf_field()}}

                                        <input id="id_current_{{$lang222->id}}" name="id" type="hidden">
                                        <input id="category_portfolio_id_{{$lang222->id}}" name="category_portfolio_id" type="hidden">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $(document).on("click",".get_content_language",function(){
                ...
                var id = $("#id").val();
                $.ajax({
                    url: "{{route('dashboard_category_portfolio_translate.get_data_by_id')}}",
                    method: "get",
                    data: {
                        "id": id,
                    ...
                    success: function (result) {
                        if (result.success != null) {
                            ...
                            $('#category_portfolio_id_' + result.success.language_id).val(result.success.category_portfolio_id);
                        }
                        else{
                            $('#category_portfolio_id_'+language_id).val(id);
                        }
                    }
                });
            });
        ...
        var Render_Data = function (id) {
            $.ajax({
                url: "{{route('dashboard_category_portfolio.get_data_by_id')}}",
                method: "get",
                data: {
                    "id": id,
                },
                ...
                success: function (result) {
                    if (result.success != null) {
                        ...
                        window.location.href = "{{route('dashboard_category_portfolio.index')}}";
                    }
                }
            });
        };
