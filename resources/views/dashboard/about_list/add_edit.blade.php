...
@section('create_btn'){{route('dashboard_about_list.index')}}@endsection
                        ...
                        <div class="tab-content" id="myTabContent">
                            <div class="tab-pane fade show active" id="home" role="tabpanel"
                                ...
                                <form class="ajaxForm users" enctype="multipart/form-data" data-name="users"
                                      action="{{route('dashboard_about_list.post_data')}}" method="post">
                                    {{csrf_field()}}
                                    ...
                                    <div class="modal-footer">
                                        ...
                                        <a href="{{route('dashboard_about_list.index')}}" class="btn btn-default">
                                            {{$lang->Close}}
                                        </a>
                            ...
                            @if($langauges->where('dir','!=',$select_lan_choice->dir)->count() > 0)
                                ...
                                <hr>
                                <form class="ajaxForm translate" data-name="translate" action="{{route('dashboard_about_list_translate.post_data')}}" method="post">
                                    <div class="modal-body row">
                                        {{csrf_field()}}
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $(document).on("click",".get_content_language",function(){
                ...
                var id = $("#id").val();
                $.ajax({
                    url: "{{route('dashboard_about_list_translate.get_data_by_id')}}",
                    method: "get",
                    data: {
                        "id": id,
        ...
        var Render_Data = function (id) {
            $.ajax({
                url: "{{route('dashboard_about_list.get_data_by_id')}}",
                method: "get",
                data: {
                    "id": id,
                },
                ...
                success: function (result) {
                    if (result.success != null) {
                        ...
                        window.location.href = "{{route('dashboard_about_list.index')}}";
                    }
                }
            });
        };
